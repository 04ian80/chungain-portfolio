...
const carouselList = [
  {
    url: '/image/main/vietnam-thumbnail.png',
    alt: '베트남에서 실무진 교육 이미지',
    caption: '봉제공장에서 실무진에게 교육 진행',
  },
  { url: '/image/main/mvp-thumbnail.png', alt: '우수사원상 이미지', caption: '우수사원상 수상' },
  {
    url: '/image/main/china-thumbnail.png',
    alt: '중국 박람회 참여 이미지',
    caption: '박람회에서 참관객에게 프로그램 시연',
  },
];
...
const Timeline = () => {
  ...
  const handleClickNext = () => {
    const nextIndex = index === LAST_INDEX ? FIRST_INDEX : index + 1;
    setIndex(nextIndex);
  };

  const handleClickPrev = () => {
    const nextIndex = index === FIRST_INDEX ? LAST_INDEX : index - 1;
    setIndex(nextIndex);
  };
...
`;
const CaretLeft = styled(AiFillCaretLeft)``;
const CaretRight = styled(AiFillCaretRight)``;
const Images = styled.div<{ $index: number }>`
  display: flex;
  gap: 8px;
  overflow: scroll;
  scrollbar-width: none;
  /* transform: ${({ $index }) => `translateX(${-280 * $index}px)`}; */
  /* display: grid;
